...
class ComputeLossAuxOTA:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model
        ...
        # Build main and auxiliary targets
        ...
        aux_available = len(p) >= 2 * self.nl

        for i in range(self.nl):
            pi = p[i]  # prediction map
            ...
            if b.shape[0]:
                ...
                # Classification loss (multi-class only)
                if self.nc > 1:
                    selected_tcls = targets[i][:, 1].long()
                    t = torch.full_like(ps[:, 5:], self.cn, device=device)
                    t[range(len(selected_tcls)), selected_tcls] = self.cp
                    lcls += self.BCEcls(ps[:, 5:], t)

            # Auxiliary branch
            if aux_available and bs_aux[i] is not None and len(bs_aux[i]):
                pi_aux = p[i + self.nl]
                ...
                if self.nc > 1:
                    ...
                    t_aux[range(len(selected_tcls_aux)), selected_tcls_aux] = self.cp
