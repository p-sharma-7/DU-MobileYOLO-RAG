...
class Concat(nn.Module):
    ...
    def forward(self, x):
        for i, xi in enumerate(x):
            print(f"Concat input {i}: shape={xi.shape}")
        
        if isinstance(x, list) and len(x) > 1:
            # Determine target height and width
            target_h = max(t.shape[2] for t in x)
            target_w = max(t.shape[3] for t in x)

            # Resize mismatched tensors
            x = [
                F.interpolate(t, size=(target_h, target_w), mode='nearest')
                if (t.shape[2] != target_h or t.shape[3] != target_w) else t
                for t in x
            ]
